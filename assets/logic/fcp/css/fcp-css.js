...
DevKits.css = (function(){
	...
	var _dealCssRule = function(_fileObj){
		...
		//css源码分析
		var _cssAnalyticRst = (new DevKits.cssAnalytic()).run(fileContent);
		//获得所有选择器
	...
	var _getSelectors = function(_cssAnalyticRst){
		...
		for(var i = 0,len = _cssAnalyticRst.length;i < len;i++){
			...
			//new line
			if(item[1] == DevKits.FL.FL_NEW_LINE) {
				continue;
			}
			//device description
			else if(item[1] == DevKits.FL.CSS_DEVICE_DESC) {
				_selector = item[0];
				_pre_type_start = DevKits.FL.CSS_DEVICE_START;
				continue;
			}
			//selector
			else if(item[1] == DevKits.FL.CSS_SELECTOER) {
				_selector = item[0];
				_pre_type_start = DevKits.FL.CSS_SELECTOER_START;
				continue;
			} 
			//@import、@charset
			else if(item[1] == DevKits.FL.CSS_AT) {
				_selector = item[0];
			}
			//csstext
			else {
				...
				for(;j < len;j++) {
					...
					//@import、@charset
					if(item[1] == DevKits.FL.CSS_AT) {
						j--;
						break;
					} 
					//device description
					else if(jtem[1] == DevKits.FL.CSS_DEVICE_END && _pre_type_start == DevKits.FL.CSS_DEVICE_START) {
						_csstext.push(jtem[0]);
						break;
					}
					//selector
					else if(jtem[1] == DevKits.FL.CSS_SELECTOER_END && _pre_type_start == DevKits.FL.CSS_SELECTOER_START) {
						_csstext.push(jtem[0]);
						break;
					}
					//csstext 
		...
		jQuery.each(_selectors,function(i,currSelector){
			...
			if(currSelector.indexOf('@') > -1 || currSelector.indexOf('-moz-') > -1) {
				...
				try{
		            ...
		            _localdata[currSelector] = jQuery(currSelector + ':not("[id^=devkits-],[id^=devkits-] ' + currSelector + '")').length;
				}catch(err){
