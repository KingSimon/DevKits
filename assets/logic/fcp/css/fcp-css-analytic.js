...
DevKits.cssAnalytic = function(){
	...
	this.tokenAnalytic = function(){
		...
		while (true){
			...
			if ($token){
				...
				if ($token[1] === DevKits.FL.CSS_PROPERTY) {
					this._output.push([':', DevKits.FL.CSS_COLON]);
				} else if ($token[1] === DevKits.FL.CSS_VALUE) {
					this._output.push([';', DevKits.FL.CSS_SEMICOLON]);
				}
			}
		}
	};
	this.getNextToken = function(){
		...
		if ($char === "\x0a") return [$char, DevKits.FL.FL_NEW_LINE];
		//避免出现多个空格在一起的情况
		...
		if ($char === '@'){
			...
			switch (this._pre_type){
				case DevKits.FL.CSS_DEVICE_DESC :
					this._pre_type = DevKits.FL.CSS_DEVICE_START;
					return [$char, DevKits.FL.CSS_DEVICE_START];
				default : 
					this._pre_type = DevKits.FL.CSS_SELECTOER_START;
					return [$char, DevKits.FL.CSS_SELECTOER_START];
			}
		}else if ($char === '}'){
			switch (this._pre_type){
				case DevKits.FL.CSS_SELECTOER_END:
					this._pre_type = DevKits.FL.CSS_DEVICE_END;
					return [$char, DevKits.FL.CSS_DEVICE_END];
				default: 
					for(var $i=this._output.length-1;$i>=0;$i--){
						var $item = this._output[$i];
						if($item[1] === DevKits.FL.CSS_SELECTOER_START){
							this._pre_type = DevKits.FL.CSS_SELECTOER_END;
							return [$char, DevKits.FL.CSS_SELECTOER_END];
						}else if($item[1] === DevKits.FL.CSS_DEVICE_START){
							this._pre_type = DevKits.FL.CSS_DEVICE_END;
							return [$char, DevKits.FL.CSS_DEVICE_END];
						}
					}
		...
		switch (this._pre_type){
			case DevKits.FL.CSS_SELECTOER_START :
			case DevKits.FL.CSS_VALUE :
				$result = this._getPropertyToken($char);
				this._pre_type = DevKits.FL.CSS_PROPERTY;
				return $result;
			case DevKits.FL.CSS_PROPERTY :
				$result = this._getValueToken($char);
				this._pre_type = DevKits.FL.CSS_VALUE;
				return $result;
			case DevKits.FL.CSS_DEVICE_START:
		...
		return [$char, DevKits.FL.CSS_NORMAL];
	};
	...
	this._getAtToken = function($char){
		...
		if (this.content[this.parsePos] === ';'){
			$resultString += ';';
			this.parsePos++;
			return [$resultString.trim(), DevKits.FL.CSS_AT];
		}
		this._pre_type = DevKits.FL.CSS_DEVICE_DESC;
		return [$resultString.trim(), DevKits.FL.CSS_DEVICE_DESC];
	};
	...
	this._getCommentToken = function($char, $fromSelector){
		...
		return ['/*' + $resultString + '*/' , DevKits.FL.CSS_COMMENT];
	};
	...
	this._getSelectorToken = function($char){
		...
		return [$resultString.trim(), DevKits.FL.CSS_SELECTOER];
	};
	...
	this._getPropertyToken = function($char){
		...
		return [$resultString.trim().toLowerCase(), DevKits.FL.CSS_PROPERTY];
	};
	...
	this._getValueToken = function($char){
		...
		return [$resultString, DevKits.FL.CSS_VALUE];
	};
