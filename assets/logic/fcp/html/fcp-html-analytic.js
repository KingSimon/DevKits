...
DevKits.htmlAnalytic = function(){
	...
	this.run = function($content, $type){
		...
		if (this.content.indexOf('<?xml') > -1){
			return [[$content, DevKits.FL.HTML_XML]];
		}
	...
	this.tokenAnalytic = function(){
		...
		while (true){
			...
			if ($token){
				if ($token[1] === DevKits.FL.FL_EOF) break;
	...
	this.getNextToken = function(){
		...
		if ($char === "\x0a"){
			return [$char, DevKits.FL.FL_NEW_LINE];
		}
		...
		if ($result) return $result;
		return [$char, DevKits.FL.FL_NORMAL];
	};
	...
	this._getTagToken = function($char){
		...
		if ($resultString[1] === '!'){
			if ($resultString.indexOf('[if') > -1){
				...
				return [$resultString, DevKits.FL.HTML_IE_HACK_START];
			}else if ($resultString.indexOf('[[endif') > -1){
				return [$resultString, DevKits.FL.HTML_IE_HACK_EDN];
			}else if (this._checkEqual($resultString, 2, 7, 'doctype')){
				return [$resultString, DevKits.FL.HTML_DOC_TYPE];
			}else if(this._checkEqual($resultString, 4, 6, 'status')){
				$resultString += this._getUnformated('-->', $resultString);
				return [$resultString, DevKits.FL.HTML_STATUS_OK];
			}else {
				$resultString += this._getUnformated('-->', $resultString);
				return [$resultString, DevKits.FL.HTML_COMMENT];
			}
		}
		if (this._checkEqual($resultString, 0, 7, '<script')){
			return [$resultString, DevKits.FL.HTML_JS_START];
		}else if (this._checkEqual($resultString, 0, 9, '</script>')){
			return [$resultString, DevKits.FL.HTML_JS_END];
		}else if (this._checkEqual($resultString, 0, 6, '<style')){
			return [$resultString, DevKits.FL.HTML_CSS_START];
		}else if (this._checkEqual($resultString, 0, 8, '</style>')){
			return [$resultString, DevKits.FL.HTML_CSS_END];
		}else if (this._checkEqual($resultString, 0, 9, '<textarea')){
			return [$resultString, DevKits.FL.HTML_TEXTAREA_START];
		}else if (this._checkEqual($resultString, 0, 11, '</textarea>')){
			return [$resultString, DevKits.FL.HTML_TEXTAREA_END];
		}else if (this._checkEqual($resultString, 0, 4, '<pre')){
			return [$resultString, DevKits.FL.HTML_PRE_START];
		}else if (this._checkEqual($resultString, 0, 6, '</pre>')){
			return [$resultString, DevKits.FL.HTML_PRE_END];
		}
		if (this._checkEqual($resultString, 0, 2, '</')){
			return [$resultString, DevKits.FL.HTML_TAG_END];
		}
		return [$resultString, DevKits.FL.HTML_TAG_START];
	};
	...
	this._getContentToken = function($char){
		...
		return [$resultString, DevKits.FL.HTML_CONTENT];
	};
	...
	this._getScriptOrStyleContent = function($char, $type){
		...
		return [$resultString.trim(), $type === 1 ? DevKits.FL.HTML_JS_CONTENT : DevKits.FL.HTML_CSS_CONTENT];
	};
	...
	this._getTextareaOrPreContent = function($char, $type){
		...
		return [$resultString.trim(), $type === 1 ? DevKits.FL.HTML_TEXTAREA_CONTENT : DevKits.FL.HTML_PRE_CONTENT];
	};
	...
	this.getTagAttributes = function($tagContent){
		...
		if ($tagContent.substr( 0, 2) === '</') {
			return [
				DevKits.FL.HTML_TAG_END,
				$tagContent.substr(2, $tagContent.length - 3).trim()
			];
		}
		//tag start
		var $result = [DevKits.FL.HTML_TAG_START, '', []];
	...
	this.getUnclosedTags = function(str){
	    ...
	    for(var i = 0,len = analyticRst.length;i < len;i++){
	    	if(analyticRst[i][1] === DevKits.FL.HTML_PRE_START ||
	    		analyticRst[i][1] === DevKits.FL.HTML_PRE_END ||
	    		analyticRst[i][1] === DevKits.FL.HTML_TEXTAREA_START ||
	    		analyticRst[i][1] === DevKits.FL.HTML_TEXTAREA_END ||
	    		analyticRst[i][1] === DevKits.FL.HTML_TAG_START ||
	    		analyticRst[i][1] === DevKits.FL.HTML_TAG_END ||
	    		analyticRst[i][1] === DevKits.FL.HTML_XML) {
	    ...
	    for(var i = 0,len = rawHtml.length;i < len;i++) {
	    	//开始标签
	    	if(rawHtml[i][1] !== DevKits.FL.HTML_PRE_END &&
	    			rawHtml[i][1] !== DevKits.FL.HTML_TEXTAREA_END &&
	    			rawHtml[i][1] !== DevKits.FL.HTML_TAG_END) {
