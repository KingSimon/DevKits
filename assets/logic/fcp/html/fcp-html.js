...
DevKits.html = (function(){
	...
	var _detectCompatMode = function() {
		_summaryInformation.documentMode = DevKits.doctype.getDocMode();
	};
	...
	var _detectDomMaxDepth = function(dom){
		...
		//扩展屏蔽
		if(dom.id === 'devkits-tab-box' || dom.id === 'devkits-pb-mask') return;
	...
	var _detectTagUnClosed = function(){
		...
		//开始进行html代码词法分析
		var htmlInstance = new DevKits.htmlAnalytic();
