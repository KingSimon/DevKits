...
DevKits.jsAnalytic = function(){
	...
	this.getNextToken = function(){
		...
		switch (true){
			case $char === '(' || $char === '[' : return [$char, DevKits.FL.JS_START_EXPR];
			case $char === ')' || $char === ']' : return [$char, DevKits.FL.JS_END_EXPR];
			case $char === '{' : return [$char, DevKits.FL.JS_START_BLOCK];
			case $char === '}' : return [$char, DevKits.FL.JS_END_BLOCK];
			case $char === ';' : return [$char, DevKits.FL.JS_SEMICOLON];
		}
		//注释或者正则
		if ($char === '/'){
			//注释
			...
			if ($tokenCount){
				...
				$lastType = DevKits.FL.JS_START_EXPR;
			}
			if (($lastType === DevKits.FL.JS_WORD && ($lastText === 'return' || $lastText === 'to'))
				|| ($lastType === DevKits.FL.JS_START_EXPR
					|| $lastType === DevKits.FL.JS_START_BLOCK
					|| $lastType === DevKits.FL.JS_END_BLOCK
					|| $lastType === DevKits.FL.JS_OPERATOR
					|| $lastType === DevKits.FL.JS_EQUALS
					|| $lastType === DevKits.FL.JS_SEMICOLON
					|| $lastType === DevKits.FL.FL_EOF
					)){
	...
	this._getWordToken = function($char){
		...
		if ((this.content[this.parsePos] === '+' || this.content[this.parsePos] === '-')
			...
			$t = this.getNextToken();
			$char += $sign . $t[0];
			return [$char, DevKits.FL.JS_WORD];
		}
		//for in operator
		if ($char === 'in'){
			return [$char , DevKits.FL.JS_OPERATOR];
		}
		return [$char, DevKits.FL.JS_WORD];
	};
	...
	this._getCommentToken = function($char){
		...
		if($c === '*'){
			...
			if ($comment.indexOf('@cc_on') === 0){
				return ['/*' + $comment + '*/', DevKits.FL.JS_IE_CC];
			}
			if ($lineComment){
				return ['/*' + $comment + '*/', DevKits.FL.JS_INLINE_COMMENT];
			}else{
				return ['/*' + $comment + '*/', DevKits.FL.JS_BLOCK_COMMENT];
			}
		}
		//单行注释
		if ($c === '/'){
			...
			this.parsePos++;
			return [$comment, DevKits.FL.JS_COMMENT];
		}
	};
	...
	this._getQuoteToken = function($char){
		...
		while (this.content[this.parsePos] !== $sep || $escape){
			...
			this.parsePos++;
			if (this.parsePos >= this.contentLength){
				return [$resultString, DevKits.FL.JS_STRING];
			}
		}
		this.parsePos++;
		$resultString += $sep;
	...
	this._getRegexpToken = function($char){
		...
		while ($escape || $inCharClass || this.content[this.parsePos] !== $sep){
			...
			this.parsePos++;
			if (this.parsePos >= this.contentLength){
				return [$resultString, DevKits.FL.JS_REGEXP];
			}
		}
		this.parsePos++;
		$resultString += $sep;
	...
	this._getSharpVariblesToken = function($char){
		...
		if (this._is_match(this.content[this.parsePos], this._digits)){
			...
			return [$sharp, DevKits.FL.JS_WORD];
		}
	};
	...
	this._getPunctToken = function($char){
		...
		return [$char, $char === '=' ? DevKits.FL.JS_EQUALS : DevKits.FL.JS_OPERATOR];
	};
