...
DevKits.fcphelper = (function(){
	...
	var _get_issue_suggestion = function(perfix,start,end){
		...
		for(var i = start;i <= end;i++) {
			...
			addTr(DevKits.i18n.getMessage(key),DevKits.i18n.getMessage(key + '_suggestion'));
		}
	...
	var _js_getCookie = function(respData,title,allContent) {
		...
		tempArr.push('<div>' + DevKits.i18n.getMessage('msg0042',[document.cookie.getBytes()]) + '。如下是和整个站点相关的cookie：</div><br />');
		...
		//-------------------------------------------
		jQuery('#devkits-box .-f-a-c-d-' + d).on('click',function(e){
	...
	var _js_getScriptTag = function(respData,title,allContent) {
		...
		if(respData.scriptTag.scriptBlock) {
			addTr(DevKits.i18n.getMessage('msg0033',[respData.scriptTag.scriptBlock]));
		}
		
		//script src
		if(respData.scriptTag.scriptSrc) {
			addTr(DevKits.i18n.getMessage('msg0034',[respData.scriptTag.scriptSrc]));
		}
		...
		if(respData.jsMinified.count > 0) {
			...
			jQuery.each(respData.jsMinified.files,function(k,item){
				if(item.href == '#') {
					t.push(DevKits.i18n.getMessage('msg0047',[item.fileName]));
				} else {
					t.push(DevKits.i18n.getMessage('msg0062',[item.href,item.fileName]));
				}
			});
			addTr(DevKits.i18n.getMessage('msg0045',[t.join('、')]));
		}
		...
		if(respData.tangram.length > 0) {
			...
			if(t.length == 1) {
				addTr(DevKits.i18n.getMessage('msg0054',[t.join('')]));
			} else {
				addTr(DevKits.i18n.getMessage('msg0055',[t.join('、')]));
			}
		}
			...
			jQuery.each(respData.duplicatedFiles,function(i,item){
				if(item.dupFiles) {	//不同地址的文件，但内容重复
					...
					jQuery.each(item.dupFiles,function(j,f){
						t.push(DevKits.i18n.getMessage('msg0069',[f,f]));
					});
					dupFile.push(DevKits.i18n.getMessage('msg0070',[t.join('、')]));
				} else {	//同一个地址的文件，被多次引入
					dupHref.push(DevKits.i18n.getMessage('msg0068',[item.href,item.href,item.count]));
				}
			});
			if(dupHref.length) {
				txt.push(DevKits.i18n.getMessage('msg0066',['<div style="margin-left:30px;">' + dupHref.join('') + '</div>']));
			}
			if(dupFile.length) {
				txt.push(DevKits.i18n.getMessage('msg0067',['<div style="margin-left:30px;">' + dupFile.join('') + '</div>']));
			}
			addTr(txt.join(''));
		}
	...
	var _html_getHTMLDeprecatedTag = function(respData,title,allContent) {
		...
		jQuery.each(respData.HTMLBase.HTMLDeprecatedTag,function(key,item){
			...
			tempArr.push('<tr>'+
				...
				'<td class="td-content">' + 
					DevKits.i18n.getMessage('msg0004',[item]) +
					DevKits.i18n.getMessage('msg0005') + '</td></tr>');
		});
		tempArr.push('</tbody></table>');
		allContent.push(_creatResultItem({
			title : title,
			...
			tempArr.push('<tr>' +
				...
				(function(){
					...
					jQuery.each(item,function(k,v){
						arr.push(DevKits.i18n.getMessage('msg0007',[ v,k ]));
					});
					return arr.join('');
				})() + DevKits.i18n.getMessage('msg0005') + '</span></td></tr>');
		});
		tempArr.push('</tbody></table>');
		allContent.push(_creatResultItem({
			title : title,
	...
	var _html_getLink = function(respData,title,allContent){
		...
		var ct = '';
		ct = '<div>' + DevKits.i18n.getMessage('msg0021',[notInHead.length,
				'head']) + '</div>';
		ct += (function(d){
			var arr = ['<table>'];
			arr.push('<thead><tr>'+
			...
			if(respData.tagInclude.length > 0) {
				jQuery.each(d,function(id,v){
					arr.push('<tr>\
						...
						<td class="td-content">' + DevKits.i18n.getMessage('msg0035',[v.inline,v.block]) + '</td>\
					</tr>');
				});
			}
			arr.push('</tbody></table>');
	...
	var _html_getDocMode = function(respData){
	    ...
	    if (respData.documentMode.hasDocType) {
			tempArr.push(DevKits.i18n.getMessage('msg0009'));
	        if (respData.documentMode.isUnusualDocType) {
	            tempArr.push(DevKits.i18n.getMessage('msg0010',[mode,quirksMode]));
	        }
	        if (respData.documentMode.hasConditionalCommentBeforeDTD || 
					respData.documentMode.hasCommentBeforeDTD) {
	            tempArr.push(DevKits.i18n.getMessage('msg0011',[mode,quirksMode]));
	        }
	        
	        if (respData.documentMode.IE == respData.documentMode.WebKit) {
	            tempArr.push(DevKits.i18n.getMessage('msg0012',[mode]));
	            if (respData.documentMode.WebKit == 'Q') {
	            	tempArr.push(DevKits.i18n.getMessage('msg0013'));
	            }
	        } else {
	            if (!respData.documentMode.isUnusualDocType) {
	                if (respData.documentMode.IE) {
	            		...
	            		tempArr.push(DevKits.i18n.getMessage('msg0014',[$1,$2]));
	            		tempArr.push(DevKits.i18n.getMessage('msg0013'));
	                } else {
	                    tempArr.push(DevKits.i18n.getMessage('msg0015'));
	                }
	            } else {
	            	tempArr.push(DevKits.i18n.getMessage('msg0013'));
	            }
	        ...
	        // 没有设置DTD
	        tempArr.push(DevKits.i18n.getMessage('msg0016',[mode]));
	        tempArr.push(DevKits.i18n.getMessage('msg0013'));
	    }
	...
	var _html_getDom = function(respData,title,allContent) {
		...
		//总数量
		addTr(DevKits.i18n.getMessage('msg0028',[respData.DOM.count]));
		
		//dom节点的最大嵌套深度
		addTr(DevKits.i18n.getMessage('msg0063',[respData.DOM.maxDepth.depth,respData.DOM.maxDepth.xpath]));
		
		//title标签检测
		if(respData.title.length == 0) {
			addTr(DevKits.i18n.getMessage('msg0049') + DevKits.i18n.getMessage('msg0052'));
		} else if(respData.title.length > 1) {
			addTr(DevKits.i18n.getMessage('msg0051') + DevKits.i18n.getMessage('msg0052'));
		} else if(!respData.title[0].isInHead){
			addTr(DevKits.i18n.getMessage('msg0050') + DevKits.i18n.getMessage('msg0052'));
		}
		...
		if(respData.imgTag.length > 0) {
			...
			jQuery.each(respData.imgTag,function(i,k){
				...
				addTr(DevKits.i18n.getMessage('msg0053',[t]));
			});
		}
		...
		if(respData.DOM.allComm.length) {
		    ...
		    if (respData.DOM.IECondComm.length) {
				addTr(DevKits.i18n.getMessage('msg0018',[respData.DOM.IECondComm.length]));
			}
			//Firefox中不支持的注释：不能出现‘--’
		    if (respData.DOM.FFNotSptComm.length) {
				addTr(DevKits.i18n.getMessage('msg0060',[respData.DOM.FFNotSptComm.length]));
			}
			
			//所有注释
			addTr(DevKits.i18n.getMessage('msg0030',[respData.DOM.allComm.length,respData.DOM.allComm.join('').getBytes()]) + DevKits.i18n.getMessage('msg0019'));
		}
		...
		if(respData.ID.count) {
			...
			jQuery.each(respData.ID.ids,function(id,v){
				id_arr.push(DevKits.i18n.getMessage('msg0064',[id,v]));
			});
			addTr(DevKits.i18n.getMessage('msg0026',[respData.ID.count,id_arr.join('，')]));
		}
		
		//问题Input
		if(respData.DOM.invalidInput.count) {
			addTr(DevKits.i18n.getMessage('msg0029',[respData.DOM.invalidInput.count]));
		}
		
		//HTML是否压缩
		if(!respData.htmlMinified) {
			addTr(DevKits.i18n.getMessage('msg0043'));
		}
		...
		if(respData.unClosedTags.length > 0) {
			...
			jQuery.each(respData.unClosedTags,function(k,item){
				t.push(DevKits.i18n.getMessage('msg0046',[item]));
			});
			addTr(DevKits.i18n.getMessage('msg0038',[t.join('、')]));
		}
	...
	var _css_getCssUsage = function(respData){
		...
		var _cssTitles = [
			DevKits.i18n.getMessage('msg0039'),
			DevKits.i18n.getMessage('msg0040'),
			DevKits.i18n.getMessage('msg0041')
		];
		...
		jQuery.each(respData.styles,function(i,style){
			...
			//创建tab
			DevKits.fcptabs.addCssTab( style.path, allContent.join('') );
		});
	};
	...
	var _css_getBackgroundImages = function(respData,allContent){
		...
		if(respData.backgroundImages.length) {
			...
			jQuery.each(respData.backgroundImages,function(i,item){
				...
				a.push('</div>');
				addTr(DevKits.i18n.getMessage('msg0057',[item.fileName,item.bgImages.length,d]) + a.join(''));
				tempInt += item.bgImages.length;
			});
		
			jQuery('#devkits-box .-x-expand-' + d).on('click',function(){
			...
			jQuery('#devkits-box .-t-c-b-' + d + ' div').on('mouseover',function(e){
		...
		allContent.push(_creatResultItem({
			title : DevKits.i18n.getMessage('msg0065'),
			count : tempInt,
			content : tempArr.join('')
		}));
	};
	...
	var _css_totalDetectResult = function(respData){
		...
		if(respData.cssMinified.count > 0) {
			...
			addTr(DevKits.i18n.getMessage('msg0044',[t.join('、')]));
		}
		...
		if(respData.expressions.length) {
			...
			jQuery.each(respData.expressions,function(i,item){
				a.push(DevKits.i18n.getMessage('msg0059',[item.fileName,item.count]));
				t += item.count;
			});
			addTr(DevKits.i18n.getMessage('msg0058',[t,a.join('、')]));
		}
		
		tempArr.push('</tbody></table>');
		
		allContent.push(_creatResultItem({
			title : DevKits.i18n.getMessage('msg0048'),
			count : tempInt,
			content : tempArr.join('')
		}));
		...
		//创建tab
		DevKits.fcptabs.addCssTab( DevKits.i18n.getMessage('msg0048'), allContent.join('') );
	};

	
	/**
	 * HTML侦测，通过调用DevKits.html.detect方法进行
	 */
	var _detectHTML = function(){
		DevKits.html.detect(function(respData){
			var content = [];
			
			//html.HTMLDeprecatedTag
			_html_getHTMLDeprecatedTag(respData,DevKits.i18n.getMessage('msg0006'),content);
	
			//html.HTMLDeprecatedAttribute
			_html_getHTMLDeprecatedAttribute(respData,DevKits.i18n.getMessage('msg0008'),content);
	
			//LINK
			_html_getLink(respData,DevKits.i18n.getMessage('msg0022'),content);
	        
			//获取标签包含关系
			_html_getTagIncludeCase(respData,DevKits.i18n.getMessage('msg0036'),content);
			
	        //其他DOM相关处理
			_html_getDom(respData,DevKits.i18n.getMessage('msg0020'),content);
			
			//创建tab 
			DevKits.fcptabs.addHtmlTab(content.join(''));
		});
	};
	
	/**
	 * CSS侦测，通过调用DevKits.css.detect方法进行
	 */
	var _detectCSS = function(){
		DevKits.css.detect(function(respData){
	...
	/**
	 * Javascript侦测，通过调用DevKits.js.detect方法进行
	 */
	var _detectJavascript = function(){
		DevKits.js.detect(function(respData){
			var allContent = [];
			
			//cookies
			_js_getCookie(respData,DevKits.i18n.getMessage('msg0031'),allContent);
			
			//script相关检测
			_js_getScriptTag(respData,DevKits.i18n.getMessage('msg0032'),allContent);
			
			//创建tab
			DevKits.fcptabs.addJavascriptTab(allContent.join(''));
		});
	};
	...
	var _addIssueSuggestionTab = function(){
		...
		//HTML问题及建议
		jQuery(_getBtnString('查看更多HTML帮助')).appendTo('#devkits-tab-html').click(function(e){

			if(!jQuery('#devkits-tab-HTML-issue-sug')[0]){
				DevKits.fcptabs.addIssueSuggestionTab('HTML',_get_issue_suggestion('html',1,42));
			} 
			
			var $allTabs = jQuery( "#devkits-main-tab>div");
			var index = $allTabs.index( jQuery( '#devkits-tab-HTML-issue-sug' ) );
			// jQuery('#devkits-main-tab').tabs( "select" , index );
			jQuery('#devkits-main-tab').tabs({active:index});
		});
		
		//CSS问题及建议
		jQuery(_getBtnString('查看更多CSS帮助')).appendTo('#devkits-tab-css').click(function(e){

			if(!jQuery('#devkits-tab-CSS-issue-sug')[0]){
				DevKits.fcptabs.addIssueSuggestionTab('CSS',_get_issue_suggestion('css',1,119));
			} 

			var $allTabs = jQuery( "#devkits-main-tab>div");
			var index = $allTabs.index( jQuery( '#devkits-tab-CSS-issue-sug' ) );
			// jQuery('#devkits-main-tab').tabs( "select" , index );
            jQuery('#devkits-main-tab').tabs({active:index});
		});
		
		//Javascript问题及建议
		jQuery(_getBtnString('查看更多Javascript帮助')).appendTo('#devkits-tab-js').click(function(e){

			if(!jQuery('#devkits-tab-Javascript-issue-sug')[0]){
				DevKits.fcptabs.addIssueSuggestionTab('Javascript',_get_issue_suggestion('javascript',1,114));
			}

			var $allTabs = jQuery( "#devkits-main-tab>div");
			var index = $allTabs.index( jQuery( '#devkits-tab-Javascript-issue-sug' ) );
			// jQuery('#devkits-main-tab').tabs( "select" , index );
            jQuery('#devkits-main-tab').tabs({active:index});
		});
	};
	...
	var _detect = function(){
		//显示进度条
		DevKits.fcptabs.createProgressBar();
		...
		window.setTimeout(function(){
			...
			//检测完毕更新进度条
			DevKits.fcptabs.updateProgressBar(100);
		},100);
	};
